...
const MessagePage = () => {
  ...
  const [openEmojiPicker, setOpenEmojiPicker] = useState(false); // State for emoji picker
  ...
  const handleEmojiSelect = (emoji) => {
    setMessage(prev => ({
      ...prev,
      text: prev.text + emoji.emoji // Append the selected emoji to the message
    }));
  };
  ...
  return (
    <div style={{ backgroundImage: `url(${backgroundImage})` }} className='bg-no-repeat bg-cover'>
      ...
      <section className='h-16 bg-white flex items-center px-4'>
        ...
        <button
          onClick={() => setOpenEmojiPicker(prev => !prev)} // Toggle emoji picker
          className='flex justify-center items-center w-11 h-11 rounded-full hover:bg-primary hover:text-white mx-2'
        >
          😊
        </button>
