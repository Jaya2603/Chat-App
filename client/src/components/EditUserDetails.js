...
import toast from 'react-hot-toast'; // Corrected the import from 'taost' to 'toast'
import { useDispatch } from 'react-redux';
import { setUser } from '../redux/userSlice';

const EditUserDetails = ({ onClose, user }) => {
    const [data, setData] = useState({
        name: user?.name || '', // Ensure default values
        profile_pic: user?.profile_pic || ''
    });
    const uploadPhotoRef = useRef();
    const dispatch = useDispatch();

    // Initialize data state from user prop
    useEffect(() => {
        if (user) {
            setData({
                name: user.name,
    ...
    const handleUploadPhoto = async (e) => {
        ...
        if (file) { // Ensure a file was selected
            try {
                ...
                setData((prev) => ({
                    ...prev,
                    profile_pic: uploadPhoto?.url || '' // Handle upload failure
                }));
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                dispatch(setUser(response.data.data)); // Update user in Redux store
                onClose(); // Close the modal
            } else {
                toast.error(response.data.message || "Failed to update user details"); // Handle success but not updated case
            }
        } catch (error) {
            console.error("Error updating user:", error); // Log error for debugging
            toast.error("Failed to update user details"); // Show error to user
        }
    };
